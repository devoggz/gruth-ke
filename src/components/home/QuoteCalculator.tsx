...
const SERVICE_TYPES = [
    { value: 'construction', label: 'Construction Verification', base: [150, 400] as [number, number] },
    { value: 'land',         label: 'Land & Property',           base: [120, 280] as [number, number] },
    { value: 'wedding',      label: 'Wedding & Events',          base: [100, 220] as [number, number] },
    { value: 'business',     label: 'Business Investment',       base: [160, 400] as [number, number] },
    { value: 'materials',    label: 'Material Pricing Audit',    base: [80,  180] as [number, number] },
] as const
...
const URGENCY = [
    ...
    { value: 'priority', label: 'Priority', sub: '24-hour turnaround', surcharge: 40 },
] as const
...
interface SizeOption { value: string; label: string; desc: string; multiplier: number }

const CONSTRUCTION_STAGES: SizeOption[] = [
    { value: 'foundation', label: 'Foundation / Slab', desc: 'Earthworks, concrete & early structure', multiplier: 1.0 },
    { value: 'shell',      label: 'Shell / Walling',   desc: 'Brickwork, columns & roofing stage',    multiplier: 1.4 },
    { value: 'fitout',     label: 'Full Fit-out',       desc: 'Plastering, finishes & completion',     multiplier: 1.9 },
]

const LAND_SIZES: SizeOption[] = [
    { value: 'plot',   label: 'Single Plot',  desc: 'Up to ½ acre residential',       multiplier: 1.0 },
    { value: 'parcel', label: 'Land Parcel',  desc: '1–5 acres, title deed check',    multiplier: 1.5 },
    { value: 'tract',  label: 'Large Tract',  desc: '5+ acres or commercial land',    multiplier: 2.0 },
]

const DEFAULT_SIZES: SizeOption[] = [
    { value: 'small',  label: 'Small',  desc: 'Single vendor / 1-day event',  multiplier: 1.0 },
    { value: 'medium', label: 'Medium', desc: 'Multi-vendor / 2–3 days',      multiplier: 1.4 },
    { value: 'large',  label: 'Large',  desc: 'Complex / multi-location',     multiplier: 1.9 },
]
...
interface EstimateRow { label: string; low: number; high: number }
interface Estimate    { low: number; high: number; breakdown: EstimateRow[] }

function useEstimate(
    service: ServiceValue, county: CountyValue,
    urgency: UrgencyValue, size: string, addons: AddonValue[],
): Estimate {
    return useMemo(() => {
        ...
        if (cty.travel > 0)      breakdown.push({ label: `${cty.label} travel`, low: cty.travel, high: cty.travel })
        if (urg.surcharge > 0)   breakdown.push({ label: 'Priority surcharge',  low: urg.surcharge, high: urg.surcharge })
        addons.forEach(a => {
                ...
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 items-start">
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mt-2">
                                {SERVICE_TYPES.map(svc => (
                                    <button key={svc.value} onClick={() => setService(svc.value)}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div>
                                ...
                                <div className="relative mt-2">
                                    <select value={county} onChange={e => setCounty(e.target.value as CountyValue)}
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-3 gap-2.5 mt-2">
                                    {getSizeOptions(service).map(p => (
                                        <button key={p.value} onClick={() => setSize(p.value)}
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mt-2">
                                {ADDONS.map(a => {
                                    ...
                                    return (
                                        <button key={a.value} onClick={() => toggleAddon(a.value)}
                    ...
                    <span className="font-display text-3xl font-bold text-white tracking-tight">
                      {f(estimate.low)}
                                        ...
                                        <span className="text-charcoal-600 font-display text-2xl mx-2">–</span>
                                        <span className="font-display text-3xl font-bold text-white tracking-tight">
                      {f(estimate.high)}
                    </span>
                                    </>
                            ...
                            <p className="text-xs text-charcoal-500 mb-6">
                                {currency === 'KES' ? 'Kenyan Shillings' : currency === 'EUR' ? 'Euros' : 'US Dollars'} · indicative, incl. VAT
                            </p>
                      ...
                      {row.low === row.high ? f(row.low) : `${f(row.low)} – ${f(row.high)}`}
                    </span>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="bg-white/[0.04] border border-white/10 rounded-2xl p-5 space-y-3.5">
                            {[
                                ...
                                <div key={i} className="flex items-start gap-3 text-xs text-charcoal-400 leading-relaxed">
                                    <span className="text-charcoal-500 flex-shrink-0 mt-0.5">{item.icon}</span>
                                    {item.text}
                                </div>
                            ))}
                        </div>
                        ...
                        <p className="text-[11px] text-charcoal-600 leading-relaxed px-1">
                            Quick estimate only. Rates are approximate — USD base, converted at indicative rates. Final pricing confirmed after project brief. May vary based on scope, access, and complexity.
                        </p>
                    </div>
