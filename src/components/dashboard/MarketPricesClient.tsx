    ...
    return (
        <div className="space-y-8">
                ...
                <div className="card p-6 space-y-5">
                    ...
                    {!comparisonMaterial ? (
                        <div className="flex flex-col items-center justify-center h-56 text-charcoal-300 gap-3">
                            ...
                            <p className="text-sm">Pick a material above to compare prices across counties</p>
                        </div>
                        ...
                        <>
                                    ...
                                    return (
                                        <>
                                            <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3">
                                                <p className="text-[10px] font-semibold text-emerald-600 uppercase tracking-widest">Cheapest</p>
                                                <p className="font-display font-bold text-emerald-700 text-lg mt-0.5">
                                                    KES {cheapest.price.toLocaleString()}
                                                </p>
                                                <p className="text-xs text-emerald-600 mt-0.5">{cheapest.county}</p>
                                            </div>
                                            <div className="bg-charcoal-50 border border-charcoal-100 rounded-xl p-3">
                                                <p className="text-[10px] font-semibold text-charcoal-500 uppercase tracking-widest">Average</p>
                                                <p className="font-display font-bold text-charcoal-800 text-lg mt-0.5">
                                                    KES {avg.toLocaleString()}
                                                </p>
                                                <p className="text-xs text-charcoal-400 mt-0.5">across {comparisonData.length} counties</p>
                                            </div>
                                            <div className="bg-red-50 border border-red-100 rounded-xl p-3">
                                                <p className="text-[10px] font-semibold text-red-500 uppercase tracking-widest">Priciest</p>
                                                <p className="font-display font-bold text-red-600 text-lg mt-0.5">
                                                    KES {priciest.price.toLocaleString()}
                                                </p>
                                                <p className="text-xs text-red-400 mt-0.5">{priciest.county}</p>
                                            </div>
                                        </>
                                    );
                                })()}
                            ...
                            <p className="text-xs text-charcoal-400">
                                Orange bar = currently selected county ({selectedCountyName}). Prices in KES.
                            </p>
                        </>
                    )}
                </div>
                                    ...
                                    <div className="hidden sm:block overflow-x-auto">
                                        <table className="w-full text-sm">
                                            ...
                                            <tr className="text-left border-b border-charcoal-100">
                                                <th className="px-5 py-3 text-xs font-semibold text-charcoal-400 uppercase tracking-widest">Material</th>
                                                <th className="px-5 py-3 text-xs font-semibold text-charcoal-400 uppercase tracking-widest">Unit</th>
                                                <th className="px-5 py-3 text-xs font-semibold text-charcoal-400 uppercase tracking-widest text-right">Price (KES)</th>
                                                <th className="px-5 py-3 text-xs font-semibold text-charcoal-400 uppercase tracking-widest">Trend</th>
                                                <th className="px-5 py-3 text-xs font-semibold text-charcoal-400 uppercase tracking-widest">Source</th>
                                                <th className="px-5 py-3 text-xs font-semibold text-charcoal-400 uppercase tracking-widest">Updated</th>
                                            </tr>
                                            </thead>
                                            <tbody className="divide-y divide-charcoal-50">
                                            {rows.map((row) => (
                                                <tr key={row.id} className="hover:bg-orange-50/40 transition-colors">
                                                    <td className="px-5 py-3.5 font-medium text-charcoal-900">{row.materialName}</td>
                                                    <td className="px-5 py-3.5 text-charcoal-500">{row.unit}</td>
                                    ...
                                    <div className="sm:hidden divide-y divide-charcoal-100">
                                        {rows.map((row) => (
                                            <div key={row.id} className="px-4 py-3.5 flex items-center justify-between gap-3">
                                                <div>
                                                    <div className="font-medium text-charcoal-900 text-sm">{row.materialName}</div>
                                                    <div className="text-xs text-charcoal-400 mt-0.5">{row.unit} · {row.sourceName}</div>
                                                </div>
            ...
            <p className="text-xs text-charcoal-400 leading-relaxed border-t border-charcoal-100 pt-4">
                <strong>Disclaimer:</strong> Prices are indicative, sourced from publicly accessible supplier
                data and GRUTH field surveys. Actual prices may vary by quantity, supplier, and market
                conditions. Significant deviations (30%+) from these figures should be queried with your
