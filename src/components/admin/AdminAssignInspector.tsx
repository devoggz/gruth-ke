...
interface Inspector { id: string; name: string | null; email: string }
interface Props { projectId: string; currentInspectorId: string | null; inspectors: Inspector[] }

export default function AdminAssignInspector({ projectId, currentInspectorId, inspectors }: Props) {
    ...
    return (
        <div className="flex items-center gap-3 flex-wrap">
            ...
            <button onClick={handleSave} disabled={loading || selected === (currentInspectorId ?? '')}
            ...
            {current && (
                <div className="text-xs text-charcoal-500 w-full">
                    Currently: <span className="font-semibold text-charcoal-800">{current.name}</span> ({current.email})
                </div>
            )}
        </div>
    )
