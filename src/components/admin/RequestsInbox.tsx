        ...
        <div className="fixed inset-0 z-50 flex">
            ...
            <div className="w-full max-w-lg bg-white h-full shadow-2xl flex flex-col overflow-hidden">
                ...
                <div className="flex items-start justify-between px-6 py-5 border-b border-charcoal-100">
                    <div>
                        <h3 className="font-display font-bold text-charcoal-950 text-lg">{request.name}</h3>
                        <p className="text-xs text-charcoal-400 mt-0.5">{request.serviceType} · {request.projectLocation}</p>
                    </div>
                    <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-charcoal-100 transition-colors text-charcoal-400 hover:text-charcoal-700">
                ...
                <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {[
                            ...
                            <div key={label} className="bg-charcoal-50 rounded-xl p-3.5">
                                <p className="text-[10px] font-bold text-charcoal-400 uppercase tracking-wider mb-1">{label}</p>
                                {href ? (
                                    <a href={href} className="text-sm font-medium text-orange-600 hover:text-orange-700 truncate block">{value}</a>
                                ) : (
                    ...
                    {/* Description */}
                    <div>
                        <p className="text-xs font-bold text-charcoal-400 uppercase tracking-wider mb-2">Description</p>
                    ...
                    {!isConverted && (
                        <div>
                            <p className="text-xs font-bold text-charcoal-400 uppercase tracking-wider mb-3">Update Status</p>
                            <div className="flex flex-wrap gap-2">
                                {STATUS_ORDER.filter(s => s !== 'CONVERTED').map(s => (
                                    <button key={s} onClick={() => handleStatus(s)} disabled={pending}
                    ...
                    {isConverted && convertedProjectId ? (
                        ...
                        <div className="border border-charcoal-200 rounded-xl p-4 space-y-3">
                            <p className="text-xs font-bold text-charcoal-400 uppercase tracking-wider">Convert to Project</p>
                            <p className="text-xs text-charcoal-500">
                                Creates a live project + client account (if not existing). Optionally assign an inspector immediately.
                            </p>

                            <div>
                                <label className="text-xs font-semibold text-charcoal-500 block mb-1.5">Assign inspector (optional)</label>
                ...
                <div className="px-6 py-4 border-t border-charcoal-100 flex gap-2.5">
                    <a href={`mailto:${request.email}?subject=Re: Your GRUTH Verification Request — ${request.serviceType}`}
                    ...
                    {request.phone && (
                        <a href={`https://wa.me/${request.phone.replace(/\D/g, '')}?text=${encodeURIComponent(`Hi ${request.name.split(' ')[0]}, this is GRUTH regarding your ${request.serviceType} verification request.`)}`}
                ...
                {loading ? (
                        ...
                        {filtered.map(req => (
                            <button key={req.id} onClick={() => setSelected(req)}
                                ...
                                <div className="w-9 h-9 bg-charcoal-950 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                                    <span className="text-white text-xs font-bold">{req.name.charAt(0).toUpperCase()}</span>
                                </div>
                      ...
                      <span className="font-semibold text-charcoal-900 text-sm group-hover:text-orange-600 transition-colors">
                        ...
                        {req.email} · {req.country}
                                        ...
                                        <div className="flex items-center gap-2.5 flex-shrink-0">
                                            ...
                                            <span className="text-[11px] text-charcoal-400">{timeAgo(req.createdAt)}</span>
                                        </div>
                                    </div>
                    ...
                    <span className="text-xs font-medium text-charcoal-600 bg-charcoal-100 px-2 py-0.5 rounded-full">
                      {req.serviceType}
                                        ...
                                        <span className="text-xs text-charcoal-400 truncate">{req.projectLocation}</span>
                                    </div>
