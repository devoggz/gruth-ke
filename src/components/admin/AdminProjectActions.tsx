...
const STATUSES = ['PENDING', 'ACTIVE', 'ON_HOLD', 'COMPLETED', 'CANCELLED'] as const
                        ...
                        {STATUSES.map(s => (
                            <button key={s} onClick={() => handleStatusChange(s)}
                                    className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium transition-colors hover:bg-charcoal-50 ${
                                        s === status ? 'text-charcoal-950 bg-charcoal-50' : 'text-charcoal-600'
                                    }`}>
                <span className={`w-2 h-2 rounded-full flex-shrink-0 ${
                    s === 'ACTIVE'    ? 'bg-emerald-500'
                        : s === 'PENDING'   ? 'bg-amber-500'
                            : s === 'ON_HOLD'   ? 'bg-orange-400'
                                : s === 'COMPLETED' ? 'bg-blue-500'
                                    : 'bg-charcoal-300'
                }`} />
                                {s.replace('_', ' ')}
                                {s === status && (
                                    <svg className="w-3.5 h-3.5 text-charcoal-400 ml-auto" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
