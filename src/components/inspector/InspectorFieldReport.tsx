...
const QUALITY = [
    { value: 'POOR',      label: 'Poor',      color: 'border-red-400 bg-red-50 text-red-700' },
    { value: 'FAIR',      label: 'Fair',       color: 'border-amber-400 bg-amber-50 text-amber-700' },
    { value: 'GOOD',      label: 'Good',       color: 'border-blue-400 bg-blue-50 text-blue-700' },
    { value: 'EXCELLENT', label: 'Excellent',  color: 'border-emerald-400 bg-emerald-50 text-emerald-700' },
]
        ...
        <div className="flex gap-1">
            {[1,2,3,4,5].map(n => (
                <button key={n} type="button"
                    ...
                    <svg className={`w-7 h-7 transition-colors ${(hovered || value) >= n ? 'text-orange-400' : 'text-charcoal-200'}`}
...
export default function InspectorFieldReport({ projectId, inspectionId, inspectorName }: Props) {
    ...
    const [newMedia, setNewMedia] = useState({ url: '', filename: '', type: 'PHOTO' as const, caption: '' })
    ...
    if (submitted) return (
        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-8 text-center">
            ...
            <h3 className="font-semibold text-emerald-900 text-lg mb-1">Report submitted</h3>
            <p className="text-emerald-700 text-sm">The client has been notified and can view your findings in their dashboard.</p>
        </div>
    )
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-7">
                        ...
                        {(['IN_PROGRESS', 'COMPLETED'] as const).map(s => (
                            <button key={s} type="button" onClick={() => set('status', s)}
                                    className={`flex-1 py-2.5 px-3 rounded-xl border text-sm font-semibold transition-all ${
                                        form.status === s ? 'border-orange-400 bg-orange-50 text-orange-700' : 'border-charcoal-200 text-charcoal-600 hover:border-orange-200'
                                    }`}>
            ...
            <div>
                ...
                <p className="text-xs text-charcoal-400 mb-2">A brief overview the client will see first — 2–4 sentences.</p>
            ...
            <div>
                ...
                <p className="text-xs text-charcoal-400 mb-2">Technical findings — measurements, issues found, conditions observed. Be specific.</p>
                <textarea rows={6} value={form.observations} onChange={e => set('observations', e.target.value)}
            ...
            {/* Recommendations */}
            <div>
                <label className="label">Recommendations <span className="font-normal text-charcoal-400 text-xs">optional</span></label>
                <textarea rows={4} value={form.recommendations} onChange={e => set('recommendations', e.target.value)}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        {QUALITY.map(q => (
                            <button key={q.value} type="button" onClick={() => set('workQuality', q.value)}
                                    className={`py-2 px-3 rounded-lg border text-sm font-semibold transition-all ${
                                        form.workQuality === q.value ? q.color + ' shadow-sm' : 'border-charcoal-100 text-charcoal-600 hover:border-charcoal-300'
                                    }`}>
                                {q.label}
                            </button>
                        ))}
            ...
            <div className="flex items-center gap-4 p-4 bg-charcoal-50 rounded-xl">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center cursor-pointer flex-shrink-0 transition-all ${
                    ...
                    {form.safetyCompliance ? (
                        ...
                        <svg className="w-3.5 h-3.5 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M18 6L6 18M6 6l12 12"/></svg>
                    )}
                </div>
                <div>
                    <p className="font-semibold text-charcoal-900 text-sm">Safety Compliance</p>
                    <p className={`text-xs ${form.safetyCompliance ? 'text-emerald-600' : 'text-red-500'}`}>
                        {form.safetyCompliance ? 'Site meets safety requirements' : 'Safety concerns noted — see observations'}
                    </p>
                </div>
            </div>
            ...
            {/* Next steps */}
            <div>
                <label className="label">Next Steps <span className="font-normal text-charcoal-400 text-xs">optional</span></label>
            ...
            <div>
                ...
                <p className="text-xs text-charcoal-400 mb-3">
                    Paste URLs of photos you've uploaded to cloud storage (Cloudinary, Uploadthing, etc.).
                </p>

                {mediaItems.length > 0 && (
                    <div className="space-y-2 mb-3">
                        {mediaItems.map((m, i) => (
                            <div key={i} className="flex items-center gap-3 bg-charcoal-50 rounded-lg px-3 py-2.5">
                                <span className="text-lg flex-shrink-0">{m.type === 'PHOTO' ? '🖼️' : m.type === 'VIDEO' ? '🎬' : '📄'}</span>
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm font-medium text-charcoal-800 truncate">{m.filename}</p>
                                    {m.caption && <p className="text-xs text-charcoal-400 truncate">{m.caption}</p>}
                                </div>
                ...
                <div className="border border-charcoal-100 rounded-xl p-4 space-y-3">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <input type="url" value={newMedia.url} onChange={e => setNewMedia(p => ({ ...p, url: e.target.value }))}
                               className="input-field sm:col-span-2" placeholder="https://... (image/video/doc URL)"/>
                        <select value={newMedia.type} onChange={e => setNewMedia(p => ({ ...p, type: e.target.value as any }))}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <input type="text" value={newMedia.filename} onChange={e => setNewMedia(p => ({ ...p, filename: e.target.value }))}
                               className="input-field" placeholder="filename.jpg"/>
                        <input type="text" value={newMedia.caption} onChange={e => setNewMedia(p => ({ ...p, caption: e.target.value }))}
                               className="input-field sm:col-span-2" placeholder="Caption (optional)"/>
                    </div>
