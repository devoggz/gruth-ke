...
export default function UserMenu({ name, email, role }: UserMenuProps) {
    ...
    const settingsHref =
        role === 'ADMIN'     ? '/admin/settings' :
            role === 'INSPECTOR' ? '/inspector/settings' :
                '/dashboard/settings'
    ...
    return (
        <div ref={ref} className="relative">
            <button onClick={() => setOpen(o => !o)} className="flex items-center gap-2.5 group">
                ...
                <div className={`w-9 h-9 rounded-full flex items-center justify-center text-white text-xs font-bold transition-all ring-2 ring-transparent group-hover:ring-orange-300 ${avatarColor}`}>
                    {initials}
                </div>
            ...
            {open && (
                <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-2xl border border-charcoal-100 shadow-xl shadow-charcoal-900/10 overflow-hidden z-50">
                    ...
                    <div className="px-4 py-3.5 border-b border-charcoal-100 bg-charcoal-50/50">
                        ...
                        {role && role !== 'CLIENT' && (
                            <div className={`mt-2.5 inline-flex items-center gap-1.5 text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide ${
                                role === 'ADMIN'     ? 'text-violet-600 bg-violet-50' :
                                    role === 'INSPECTOR' ? 'text-emerald-600 bg-emerald-50' : ''
                            }`}>
                <span className={`w-1.5 h-1.5 rounded-full ${
                    ...
                    <div className="py-1.5">
                        ...
                        {role === 'INSPECTOR' && (
                            <Link href="/inspector" onClick={() => setOpen(false)}
                                  className="flex items-center gap-3 px-4 py-2.5 text-sm text-charcoal-700 hover:bg-charcoal-50 transition-colors">
                                <svg className="w-4 h-4 text-charcoal-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
