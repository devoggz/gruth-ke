...
function Bubble({ msg }: { msg: Message }) {
    ...
    return (
        <div className={`flex ${isClient ? 'justify-end' : 'justify-start'} group`}>
            <div className={`max-w-[75%] flex flex-col ${isClient ? 'items-end' : 'items-start'} gap-1`}>
                {!isClient && (
                    <span className="text-[10px] font-semibold text-charcoal-400 px-1">{msg.senderName}</span>
                )}
    ...
    return (
        <button
            ...
            className={`w-full text-left p-4 rounded-xl transition-all hover:bg-charcoal-50 ${
                active ? 'bg-orange-50 border border-orange-200' : 'border border-transparent'
            }`}
        >
            <div className="flex items-start justify-between gap-2 mb-1">
                <h3 className={`text-sm font-semibold truncate ${active ? 'text-orange-700' : 'text-charcoal-900'}`}>
                    {thread.projectName}
                </h3>
            ...
            <p className="text-[10px] text-charcoal-300 mt-1">{formatRelativeDate(thread.lastMessageAt)}</p>
        </button>
    )
}
...
export default function MessagesPage() {
    ...
    async function send() {
        ...
        // Optimistic update
        setActiveThread(prev => prev ? { ...prev, messages: [...prev.messages, optimistic] } : prev)
        setInput('')
        try {
            await fetch('/api/messages/send', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ projectId: activeThread.projectId, content: optimistic.content }),
            })
            await fetchThreads()
        } catch {
    ...
    return (
        <div className="pb-12">
            <div className="mb-6">
                <h1 className="font-display text-2xl font-bold text-charcoal-950 tracking-tight">Messages</h1>
                <p className="text-charcoal-500 text-sm mt-1">Direct line to your GRUTH inspection team</p>
            </div>
            ...
            {threads.length === 0 ? (
                <div className="card p-16 text-center max-w-lg mx-auto">
                    ...
                    <h3 className="font-display font-semibold text-charcoal-900 mb-2">No messages yet</h3>
                    <p className="text-charcoal-500 text-sm">
                        Once you submit a verification request, your assigned inspector will reach out here.
                    </p>
                </div>
            ) : (
                <div className="card overflow-hidden" style={{ height: 'calc(100vh - 220px)', minHeight: '480px' }}>
                        ...
                        {activeThread ? (
                            <div className="flex-1 flex flex-col min-w-0">
                                ...
                                <div className="px-5 py-4 border-b border-charcoal-100 flex items-center justify-between gap-3">
                                    <div>
                                        <h2 className="font-semibold text-charcoal-950 text-sm">{activeThread.projectName}</h2>
                                        <p className="text-xs text-charcoal-400 mt-0.5">{activeThread.projectType}</p>
                                    </div>
                                    <a
                                ...
                                <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
                                    {activeThread.messages.length === 0 && (
                                        <div className="text-center py-12 text-charcoal-400 text-sm">
                                            No messages yet in this thread. Send a message to your inspector.
                                        </div>
                                    )}
