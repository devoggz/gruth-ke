...
export default async function DashboardPage() {
  ...
  const [projects, unreadAlerts, unreadMessages, recentInspections, inspectionHistory] =
      await Promise.all([
        ...
        prisma.alert.count({ where: { project: { clientId: userId }, isRead: false } }),
        prisma.message.count({ where: { userId, isFromClient: false, readAt: null } }),
        ...
        prisma.inspection.findMany({
          where: {
            ...
            scheduledDate: { gte: new Date(Date.now() - 1000 * 60 * 60 * 24 * 180) },
          },
  ...
  const totalInspections = await prisma.inspection.count({ where: { project: { clientId: userId } } });
  ...
  inspectionHistory.forEach(ins => {
    ...
    const key = ws.toLocaleDateString("default", { month: "short", day: "numeric" });
      ...
      <div className="space-y-6 pb-12">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          ...
          <div className="bg-white border border-charcoal-100 rounded-2xl p-4 sm:p-5">
            ...
            <div className="text-charcoal-700 text-sm font-medium">Total Projects</div>
          ...
          <div className="bg-white border border-charcoal-100 rounded-2xl p-4 sm:p-5">
            ...
            <div className="text-charcoal-700 text-sm font-medium">Inspections</div>
            <div className="mt-1">
              <span className="text-charcoal-400 text-xs">Across all projects</span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_296px] gap-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-display text-base font-semibold text-charcoal-950">Your Projects</h2>
              <Link href="/dashboard/projects" className="text-orange-500 text-sm font-semibold hover:text-orange-600 transition-colors">
                View all →
              </Link>
            </div>
            ...
            {projects.length === 0 ? (
                <div className="bg-white border border-charcoal-100 rounded-2xl p-10 sm:p-12 text-center">
                  ...
                  <h3 className="font-display font-semibold text-charcoal-900 text-sm mb-1.5">No projects yet</h3>
                  <p className="text-charcoal-500 text-sm mb-5">Submit your first verification request to get started.</p>
                  <Link
                      href="/request-verification"
                    ...
                    return (
                        ...
                        >
                          ...
                          <div className="flex items-start justify-between gap-2 mb-2">
                            ...
                            <span className={`status-badge flex-shrink-0 ${getStatusColor(project.status)}`}>
                        {project.status}
                      </span>
                          </div>
          ...
          <div className="space-y-4">
            ...
            {/* Recent activity */}
            <div className="bg-white border border-charcoal-100 rounded-2xl p-4 sm:p-5">
              <h2 className="font-display text-sm font-semibold text-charcoal-950 mb-4">Recent Activity</h2>
            ...
            {/* Quick actions */}
            <div className="bg-white border border-charcoal-100 rounded-2xl p-4 sm:p-5">
              <h3 className="text-xs font-bold text-charcoal-500 uppercase tracking-widest mb-3">Quick Actions</h3>
              <div className="space-y-0.5">
                {[
                  { label: "New verification request", href: "/request-verification",   icon: <IconPlus />,     badge: undefined },
                  { label: "All projects",             href: "/dashboard/projects",      icon: <IconFolder />,   badge: undefined },
                  { label: "Messages",                 href: "/dashboard/messages",      icon: <IconMessage />,  badge: unreadMessages },
                  { label: "Market prices",            href: "/dashboard/market-prices", icon: <IconBox />,      badge: undefined },
                  { label: "Notifications",            href: "/dashboard/notifications", icon: <IconBell />,     badge: unreadAlerts },
                  { label: "Settings",                 href: "/dashboard/settings",      icon: <IconSettings />, badge: undefined },
...
// ─── Icons ────────────────────────────────────────────────────────────────────
const IconFolder   = () => <svg className="w-4 h-4 sm:w-5 sm:h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M22 19a2 2 0 01-2 2H4a2 2 0 01-2-2V5a2 2 0 012-2h5l2 3h9a2 2 0 012 2z"/></svg>
const IconSearch   = () => <svg className="w-4 h-4 sm:w-5 sm:h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
const IconBell     = () => <svg className="w-4 h-4 sm:w-5 sm:h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 01-3.46 0"/></svg>
const IconMessage  = () => <svg className="w-4 h-4 sm:w-5 sm:h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>
