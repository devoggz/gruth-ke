...
export default async function InspectorLayout({ children }: { children: React.ReactNode }) {
    ...
    if (user?.role !== 'INSPECTOR' && user?.role !== 'ADMIN') redirect('/dashboard')
            ...
            <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
                <header className="h-16 bg-white border-b border-charcoal-100 px-6 flex items-center justify-between flex-shrink-0">
                    <div className="hidden md:flex items-center gap-2">
                        <span className="text-[10px] font-bold text-emerald-600 uppercase tracking-[0.12em]">Inspector Portal</span>
                    </div>
                    <div className="md:hidden flex items-center gap-2.5">
                        ...
                        <span className="font-display font-bold text-charcoal-950 text-sm">GRUTH Inspector</span>
                    </div>
