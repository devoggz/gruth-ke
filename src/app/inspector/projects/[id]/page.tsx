...
export default async function InspectorProjectDetailPage({ params }: PageProps) {
    ...
    const project = await prisma.project.findUnique({
        where: { id },
        include: {
            client: { select: { name: true, email: true, phone: true, country: true } },
            inspections: {
        ...
        <div className="space-y-8 pb-16">
            ...
            <div>
                <div className="flex items-center gap-1.5 text-xs text-charcoal-400 mb-3">
                    <Link href="/inspector" className="hover:text-emerald-600 transition-colors">My Projects</Link>
                    <span>/</span>
            ...
            <div className="card p-5 flex items-center gap-5">
                ...
                <div className="flex-1 min-w-0">
                    <p className="font-semibold text-charcoal-900">{project.client.name}</p>
                    <p className="text-charcoal-400 text-sm">{project.client.country} diaspora</p>
                    <div className="flex items-center gap-4 mt-1.5 flex-wrap">
                        {project.client.email && (
                            <a href={`mailto:${project.client.email}`} className="text-sm text-orange-600 hover:underline flex items-center gap-1.5">
                        ...
                        {project.client.phone && (
                            <a href={`https://wa.me/${project.client.phone.replace(/\D/g, '')}`} target="_blank" rel="noreferrer"
                ...
                <div className="text-right flex-shrink-0">
                    ...
                    <p className="font-semibold text-charcoal-800 text-sm">{project.startDate ? formatDate(project.startDate) : '—'}</p>
                </div>
            </div>
            ...
            {project.description && (
                <div className="card p-5">
                    <h2 className="font-semibold text-charcoal-900 mb-2 text-sm uppercase tracking-wide">Client Brief</h2>
                    <p className="text-charcoal-700 text-sm leading-relaxed">{project.description}</p>
                </div>
            )}
            ...
            {project.progressStages.length > 0 && (
                <div className="card p-5">
                    <h2 className="font-semibold text-charcoal-900 mb-4 text-sm uppercase tracking-wide">Project Stages</h2>
                    <div className="space-y-2.5">
                        {project.progressStages.map(s => (
                            <div key={s.id} className="flex items-center gap-3">
                                <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${s.completed ? 'bg-emerald-500' : 'bg-charcoal-100'}`}>
                                    {s.completed && <svg className="w-3 h-3 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M20 6L9 17l-5-5"/></svg>}
                                </div>
                                <span className={`text-sm ${s.completed ? 'text-charcoal-800 font-medium' : 'text-charcoal-400'}`}>{s.stageName}</span>
                                {s.completedAt && <span className="text-xs text-charcoal-300 ml-auto">{formatDate(s.completedAt)}</span>}
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {completedInspections.length > 0 && (
                <div>
                    <h2 className="font-semibold text-charcoal-900 mb-4">Previous Inspection Reports</h2>
                    <div className="space-y-5">
                        {completedInspections.map(insp => (
                            <div key={insp.id} className="card p-6">
                                <div className="flex items-start justify-between gap-4 mb-4">
                                    <div>
                                        <p className="font-semibold text-charcoal-900 text-sm">Inspection — {formatDate(insp.scheduledDate)}</p>
                                        <p className="text-charcoal-400 text-xs mt-0.5">{formatRelativeDate(insp.scheduledDate)}</p>
                                    </div>
                                    <span className={`status-badge ${getStatusColor(insp.status)}`}>{insp.status}</span>
                                </div>
                                {insp.summary && (
                                    <div className="mb-3">
                                        <p className="text-xs text-charcoal-400 mb-1 font-medium uppercase tracking-wide">Summary</p>
                                        <p className="text-charcoal-700 text-sm leading-relaxed bg-gray-50 rounded-lg p-3">{insp.summary}</p>
                                    </div>
                                )}
                                {insp.media.filter(m => m.type === 'PHOTO').length > 0 && (
                                    <div className="flex gap-2 mt-3 overflow-x-auto pb-1">
                                        {insp.media.filter(m => m.type === 'PHOTO').map(m => (
                                            <div key={m.id} className="relative w-20 h-20 rounded-lg overflow-hidden flex-shrink-0 bg-gray-100">
                                                <Image src={m.url} alt={m.caption ?? m.filename} fill className="object-cover"/>
                                            </div>
                                        ))}
                                    </div>
                                )}
                                {insp.overallRating && (
                                    <div className="flex items-center gap-2 mt-3">
                                        {[1,2,3,4,5].map(n => (
                                            <svg key={n} className={`w-4 h-4 ${n <= insp.overallRating! ? 'text-orange-400' : 'text-charcoal-200'}`} viewBox="0 0 24 24" fill="currentColor">
                                                <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
                                            </svg>
                                        ))}
                                        {insp.workQuality && <span className="text-xs text-charcoal-500 ml-1">{insp.workQuality}</span>}
                                    </div>
                                )}
                            </div>
                        ))}
            ...
            {allPhotos.length > 0 && (
                <div className="card p-5">
                    <h2 className="font-semibold text-charcoal-900 mb-4">Evidence Gallery ({allPhotos.length} photos)</h2>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                        {allPhotos.map(m => (
                            <a key={m.id} href={m.url} target="_blank" rel="noreferrer"
                               className="relative aspect-square rounded-lg overflow-hidden bg-gray-100 group">
                                <Image src={m.url} alt={m.caption ?? m.filename} fill className="object-cover group-hover:scale-105 transition-transform duration-300"/>
                                {m.caption && (
            ...
            <div className="card p-6">
                <div className="mb-6">
                    <h2 className="font-display text-xl font-bold text-charcoal-950 mb-1">
                        {latestScheduled ? 'Submit Inspection Report' : 'Log New Inspection'}
                    </h2>
                    <p className="text-charcoal-500 text-sm">
                        Your report will be immediately visible to the client in their project dashboard.
                    </p>
                </div>
