        ...
        <section className="bg-charcoal-950 py-16">
          ...
          <span className="inline-flex items-center gap-1.5 text-orange-400 text-xs font-bold tracking-widest uppercase bg-orange-500/10 border border-orange-500/20 px-3 py-1.5 rounded-full mb-5">
            ...
            <p className="text-charcoal-300 text-lg leading-relaxed max-w-xl mx-auto">
              Message us and we'll respond within 2 business hours (EAT). For urgent matters, WhatsApp is fastest.
            </p>
          </div>
        </section>
        ...
        <section className="py-16 bg-charcoal-50">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="space-y-8">
              <div>
                <h2 className="font-display text-xl font-bold text-charcoal-950 mb-5">Contact details</h2>
                  ...
                  {[
                    ...
                    },
                    {
                      ...
                      <div key={label} className="flex items-start gap-3">
                        ...
                        <div>
                          <div className="text-xs text-charcoal-400 uppercase tracking-widest font-semibold mb-0.5">{label}</div>
                          {href ? (
                              <a href={href} className="text-charcoal-800 text-sm font-medium hover:text-orange-600 transition-colors">
                                {value}
                              </a>
                          ) : (
                              <div className="text-charcoal-800 text-sm font-medium">{value}</div>
                          )}
                        </div>
                      </div>
                  ))}
              ...
              <div className="bg-white rounded-2xl border border-charcoal-100 p-5 shadow-sm">
                <div className="flex items-center gap-2 mb-3">
                  <div className={`w-2 h-2 rounded-full ${isOpen ? "bg-emerald-500 animate-pulse" : "bg-charcoal-300"}`}/>
                  <span className={`text-xs font-bold uppercase tracking-widest ${isOpen ? "text-emerald-600" : "text-charcoal-400"}`}>
                  {isOpen ? "Available now" : "Currently offline"}
                </span>
                </div>
                <div className="space-y-2 text-xs text-charcoal-500">
                  <div className="flex justify-between">
                    <span>Mon – Fri</span><span className="font-semibold text-charcoal-800">8am – 8pm EAT</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Sat – Sun</span><span className="font-semibold text-charcoal-800">9am – 5pm EAT</span>
                  </div>
                  <div className="flex justify-between border-t border-charcoal-100 pt-2 mt-2">
                    <span>Response time</span><span className="font-semibold text-orange-600">Within 2 hours</span>
                  </div>
                </div>
              </div>
              ...
              >
                ...
                <div>
                  ...
                  <div className="text-[11px] text-green-100 leading-tight">Fastest response · usually &lt;30 min</div>
                </div>
                <svg className="w-4 h-4 ml-auto opacity-70 group-hover:translate-x-0.5 transition-transform" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
            ...
            <div className="lg:col-span-2">
              {sent ? (
                  <div className="bg-white rounded-2xl p-12 text-center border border-charcoal-100 shadow-sm">
                    ...
                    <h3 className="font-display text-2xl font-bold text-charcoal-950 mb-3">Message sent!</h3>
                    <p className="text-charcoal-600 mb-1">
                      We'll get back to you at <strong>{form.email}</strong>
                    </p>
                    <p className="text-charcoal-400 text-sm">within 2 business hours (EAT).</p>
                  </div>
              ) : (
                  <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-8 border border-charcoal-100 shadow-sm space-y-5">
                    <div>
                      <h3 className="font-display text-xl font-bold text-charcoal-950 mb-1">Send a message</h3>
                      <p className="text-charcoal-500 text-sm">We read every message and respond within 2 business hours.</p>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      <div>
                        <label className="label">Your name</label>
                        <input required className="input-field" placeholder="James Mwangi"
                               value={form.name} onChange={e => set("name", e.target.value)}/>
                      </div>
                      <div>
                        <label className="label">Email address</label>
                        <input required type="email" className="input-field" placeholder="james@example.com"
                    ...
                    <button disabled={loading} type="submit" className="btn-primary w-full justify-center py-3.5 disabled:opacity-60 disabled:cursor-not-allowed">
                      {loading ? (
