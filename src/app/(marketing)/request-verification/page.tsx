...
// 4-step form: Contact → Service & Location → Details → Files & Submit
...
function buildSelectStyles(hasError = false) {
  return {
    control: (b: any, s: any) => ({
      ...b, minHeight: "46px", borderRadius: "0.5rem", fontSize: "14px",
      fontFamily: "var(--font-body,'DM Sans',system-ui,sans-serif)",
      background: "#fff",
      border: `1px solid ${s.isFocused ? "transparent" : hasError ? "#fca5a5" : "#e5e7eb"}`,
      boxShadow: s.isFocused ? (hasError ? "0 0 0 2px rgba(239,68,68,0.4)" : "0 0 0 2px rgba(249,115,22,0.5)") : "none",
    ...
    menu: (b: any) => ({ ...b, borderRadius: "0.75rem", border: "1px solid #e5e7eb", boxShadow: "0 10px 40px rgba(0,0,0,0.10)", background: "#fff", marginTop: "6px", zIndex: 60, overflow: "hidden", fontFamily: "var(--font-body,'DM Sans',system-ui,sans-serif)" }),
...
function LocationPicker({ value, onChange, error }: {
  value: string; onChange: (v: string, placeId?: string) => void; error?: string;
}) {
  ...
  const fetchPreds = useCallback((q: string) => {
    if (q.length < 3) { setPreds([]); setOpen(false); return; }
    if (!init()) return;
    setLoading(true);
    svcRef.current.getPlacePredictions(
        { input: q, sessionToken: tokenRef.current, componentRestrictions: { country: "ke" }, types: ["geocode", "establishment"] },
        (res: Prediction[] | null) => { setLoading(false); setPreds(res ?? []); setOpen((res?.length ?? 0) > 0); }
    );
  }, [init]);

  const handleType = (e: React.ChangeEvent<HTMLInputElement>) => {
    const v = e.target.value;
    setInput(v); setConfirmed(false); onChange(v);
    clearTimeout(debRef.current);
    debRef.current = setTimeout(() => fetchPreds(v), 320);
  };

  const handlePick = (p: Prediction) => {
    setInput(p.description); setConfirmed(true);
  ...
  return (
      <div ref={wrapRef} className="relative">
        ...
        <span className="absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none text-charcoal-400">
          ...
          <input type="text" value={input} onChange={handleType} autoComplete="off"
        ...
        {open && preds.length > 0 && (
            <ul className="absolute z-50 mt-1.5 w-full bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden">
              {preds.map(p => (
                  <li key={p.place_id}>
                    <button type="button" onMouseDown={e => { e.preventDefault(); handlePick(p); }}
                      ...
                      <div className="min-w-0">
                        <p className="font-semibold text-charcoal-900 text-sm truncate group-hover:text-orange-700">{p.structured_formatting.main_text}</p>
                        {p.structured_formatting.secondary_text && (
                            <p className="text-charcoal-400 text-xs mt-0.5 truncate">{p.structured_formatting.secondary_text}</p>
                        )}
                      </div>
                    </button>
                  </li>
      ...
      <div>
        ...
        >
          <div className="w-12 h-12 bg-charcoal-100 group-hover:bg-orange-100 rounded-xl flex items-center justify-center mx-auto mb-3 transition-colors">
            <svg className="w-6 h-6 text-charcoal-400 group-hover:text-orange-500 transition-colors" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
          ...
          <p className="text-xs text-charcoal-400 mt-1">Photos, PDFs, videos — up to 10 files · 50MB each</p>
        ...
        {files.length > 0 && (
            <ul className="mt-3 space-y-2">
              {files.map((f, i) => (
                  <li key={i} className="flex items-center gap-3 bg-charcoal-50 rounded-lg px-3 py-2.5 group">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-charcoal-400">{formatSize(f.size)}</p>
                    </div>
...
const SERVICE_TYPES = [
  ...
  { value: "Land & Property Verification",       icon: "🏡", label: "Land & Property" },
  { value: "Wedding / Event Verification",       icon: "💒", label: "Wedding / Event" },
  { value: "Funeral Event Oversight",            icon: "🕊️", label: "Funeral"         },
  { value: "Business & Investment Verification", icon: "📊", label: "Business"        },
...
const URGENCY = [
  ...
  { value: "standard", label: "Standard", sub: "Within 1 week",  dot: "bg-amber-400"  },
...
const Err = ({ msg }: { msg?: string }) =>
    msg ? (
        <p className="flex items-center gap-1 text-red-500 text-xs mt-1.5">
          <svg className="w-3 h-3 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h.01"/></svg>
          {msg}
        </p>
    ) : null;
                ...
                <React.Fragment key={n}>
                  ...
                  <div className="flex flex-col items-center gap-1.5 flex-shrink-0">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-200 ${
                        done ? "bg-emerald-500 text-white" : active ? "bg-orange-500 text-white shadow-lg shadow-orange-200" : "bg-charcoal-100 text-charcoal-400"
                    }`}>
                      {done ? (
                          <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M20 6L9 17l-5-5"/></svg>
                      ) : n}
                    </div>
                    <span className={`text-[11px] font-medium hidden sm:block leading-none ${active ? "text-orange-600" : done ? "text-charcoal-400" : "text-charcoal-300"}`}>
                  {STEP_LABELS[i]}
                </span>
                  </div>
                </React.Fragment>
...
export default function RequestVerificationPage() {
  ...
  const [form, setForm] = useState({
    name: "", email: "",
    phone: "" as E164Number | "",
    country: "", countryOption: null as any,
    projectLocation: "", projectLocationPlaceId: "",
    county: "", countyOption: null as any,
    serviceType: "",
    budgetOption: null as any, budgetRange: "",
    urgency: "standard",
    description: "", specificConcerns: "", onGroundContact: "",
  });
  ...
  const validators: Record<number, () => Record<string, string>> = {
    ...
    3: () => {
      ...
      else if (form.description.trim().length < 40) e.description = `${40 - form.description.trim().length} more characters needed`;
      return e;
    },
    4: () => ({}),
  };
  ...
  const back = () => { setErrors({}); setStep(s => Math.max(s - 1, 1) as 1 | 2 | 3 | 4); };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      country: form.country, projectLocation: form.projectLocation, county: form.county,
      serviceType: form.serviceType, budgetRange: form.budgetRange, urgency: form.urgency,
      specificConcerns: form.specificConcerns, onGroundContact: form.onGroundContact,
      description: [
        form.description,
      ...
      <div className="pt-16 min-h-screen flex items-center justify-center bg-charcoal-50">
        <div className="max-w-md w-full mx-auto px-4 py-20 text-center">
          <div className="w-20 h-20 bg-emerald-100 rounded-full flex items-center justify-center mx-auto mb-6 ring-8 ring-emerald-50">
            <svg className="w-10 h-10 text-emerald-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M20 6L9 17l-5-5"/></svg>
          </div>
          ...
          <p className="text-charcoal-600 leading-relaxed mb-2">
            Our team will contact <strong>{form.name.split(" ")[0]}</strong> within{" "}
            <strong>2 business hours</strong> via email and WhatsApp.
          </p>
          <p className="text-charcoal-400 text-sm mb-8">Confirmation sent to <span className="font-medium text-charcoal-600">{form.email}</span></p>
          <div className="grid grid-cols-3 gap-3 card p-5 mb-8">
            {[{ icon: "📋", label: "Request logged" }, { icon: "🔍", label: "Matching inspector" }, { icon: "📞", label: "We'll call you" }].map(({ icon, label }) => (
        ...
        <div className="pt-16">
          ...
          <section className="bg-charcoal-950 py-14">
            <div className="max-w-3xl mx-auto px-4 text-center">
              <span className="inline-flex items-center gap-1.5 text-orange-400 text-xs font-bold tracking-widest uppercase bg-orange-500/10 border border-orange-500/20 px-3 py-1.5 rounded-full mb-5">Get Started</span>
              <h1 className="font-display text-4xl sm:text-5xl font-bold text-white mb-4 tracking-tight">Request a Verification</h1>
              ...
              <div className="flex items-center justify-center gap-6 mt-7 flex-wrap">
                {[{ icon: "⚡", text: "2-hr response" }, { icon: "🔒", text: "Fully confidential" }, { icon: "📍", text: "Kenya-wide coverage" }].map(({ icon, text }) => (
                    <div key={text} className="flex items-center gap-2 text-charcoal-400 text-sm"><span>{icon}</span><span>{text}</span></div>
                ))}
              </div>
            </div>
          </section>
                  ...
                  {step === 1 && (
                      <div className="space-y-5">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-charcoal-400 mt-1.5">Used for WhatsApp updates</p>
                        </div>
                        <div>
                          <label className="label">Country of Residence *</label>
                          <Select instanceId="country-select" options={countryOptions} value={form.countryOption}
                                  onChange={opt => { set("countryOption", opt ?? null); set("country", opt?.label ?? ""); }}
                                  placeholder="Search your country…" isSearchable isClearable
                                  styles={buildSelectStyles(!!errors.country)} noOptionsMessage={() => "No country found"}
                                  menuPortalTarget={typeof document !== "undefined" ? document.body : undefined} menuPosition="fixed"/>
                          <Err msg={errors.country}/>
                        </div>
                        <button type="button" onClick={next} className="btn-primary w-full justify-center py-3.5 mt-2">
                          Continue
                  ...
                  {step === 2 && (
                      <div className="space-y-6">
                        <div className="flex items-center justify-between">
                          ...
                          <button type="button" onClick={back} className="text-sm font-medium text-charcoal-400 hover:text-charcoal-700 flex items-center gap-1.5 transition-colors flex-shrink-0">
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-1">
                            {SERVICE_TYPES.map(({ value, icon, label }) => (
                                <button key={value} type="button" onClick={() => set("serviceType", value)}
                                        className={`flex flex-col items-center gap-2 px-3 py-4 rounded-xl border text-center transition-all duration-150 ${
                                            form.serviceType === value ? "border-orange-400 bg-orange-50 shadow-sm" : "border-charcoal-100 hover:border-orange-200 hover:bg-orange-50/40"
                                        }`}>
                                  <span className="text-2xl leading-none">{icon}</span>
                                  <span className={`text-xs font-semibold leading-tight ${form.serviceType === value ? "text-orange-700" : "text-charcoal-700"}`}>{label}</span>
                                </button>
                            ))}
                          </div>
                        ...
                        <div>
                          ...
                          <Select instanceId="county-select" options={KENYA_COUNTIES} value={form.countyOption}
                                  ...
                                  styles={buildSelectStyles(!!errors.county)} noOptionsMessage={() => "No county found"}
                                  menuPortalTarget={typeof document !== "undefined" ? document.body : undefined} menuPosition="fixed"/>
                        ...
                        <div>
                          <label className="label">
                            Specific Location *{" "}
                            <span className="font-normal text-charcoal-400 text-xs">— powered by Google Maps</span>
                          </label>
                          <LocationPicker value={form.projectLocation}
                                          onChange={(val, placeId) => { set("projectLocation", val); if (placeId) set("projectLocationPlaceId", placeId); }}
                  ...
                  {step === 3 && (
                      <div className="space-y-6">
                        ...
                        <div>
                          <label className="label">
                            Specific Concerns{" "}
                            <span className="font-normal text-charcoal-400 text-xs">optional</span>
                          </label>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                          <div>
                            ...
                            <Select instanceId="budget-select" options={BUDGET_OPTIONS} value={form.budgetOption}
                                    onChange={opt => { set("budgetOption", opt ?? null); set("budgetRange", opt?.value ?? ""); }}
                                    placeholder="Select a range…" isClearable styles={buildSelectStyles(false)}
                                    menuPortalTarget={typeof document !== "undefined" ? document.body : undefined} menuPosition="fixed"/>
                            <p className="text-xs text-charcoal-400 mt-1.5">Total project cost, not the GRUTH fee.</p>
                          </div>
                          <div>
                            ...
                            <div className="flex flex-col gap-2">
                              {URGENCY.map(({ value, label, sub, dot }) => (
                                  <button key={value} type="button" onClick={() => set("urgency", value)}
                                    ...
                                    <span className={`text-sm font-semibold ${form.urgency === value ? "text-orange-700" : "text-charcoal-800"}`}>{label}</span>
                  ...
                  {step === 4 && (
                      <div className="space-y-6">
                        ...
                        <div className="rounded-2xl bg-charcoal-950 p-5">
                          ...
                          <div className="space-y-1.5 text-sm">
                            {[
                              ...
                              ["County", form.county || "—"],
                              ["Urgency", URGENCY.find(u => u.value === form.urgency)?.label ?? "—"],
                            ].map(([k, v]) => (
                                <div key={k} className="flex gap-3">
                                  <span className="text-charcoal-500 w-20 flex-shrink-0">{k}</span>
                                  <span className="text-charcoal-200 truncate">{v}</span>
                                </div>
                            ))}
                            {uploadedFiles.length > 0 && (
                                <div className="flex gap-3">
                                  <span className="text-charcoal-500 w-20 flex-shrink-0">Files</span>
                        ...
                        <button type="submit" disabled={loading}
                          ...
                          <svg className="animate-spin w-4 h-4" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" strokeDasharray="20 60"/></svg>
                          Submitting…
                        </span>
                          ) : (
                              <>Submit Verification Request <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M5 12h14M12 5l7 7-7 7"/></svg></>
                          )}
                        </button>
