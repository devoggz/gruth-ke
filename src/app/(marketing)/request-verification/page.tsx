...
// ─── Location picker (Google Places autocomplete, Kenya only) ─────────────────
  ...
  return (
      <div ref={wrapRef} className="relative">
        ...
        <span className="absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none text-charcoal-400">
          ...
          <input
              type="text" value={input} onChange={handleType} autoComplete="off"
...
const URGENCY = [
  ...
  { value: "flexible", label: "Flexible", sub: "Within 2 weeks", dot: "bg-emerald-400" },
];
...
export default function RequestVerificationPage() {
  ...
  const [form, setForm] = useState({
    ...
    serviceType: "",
    budgetOption: null as { label: string; value: string } | null,
    budgetRange: "",
    urgency: "standard",
    description: "",
    specificConcerns: "",
  ...
  const validateStep2 = () => {
    ...
    if (!form.description.trim())     e.description = "Please describe your project";
      ...
      e.description = `Please add ${40 - form.description.trim().length} more characters`;
    setErrors(e);
    return Object.keys(e).length === 0;
  };

  const nextStep = () => { if (validateStep1()) { setErrors({}); setStep(2); window.scrollTo({ top: 0, behavior: "smooth" }); } };
  const prevStep = () => { setErrors({}); setStep(1); };
        ...
        <div className="pt-16 min-h-screen flex items-center justify-center bg-charcoal-50">
          <div className="max-w-md w-full mx-auto px-4 py-20 text-center">
            ...
            <p className="text-charcoal-400 text-sm mb-8">
              Confirmation sent to <span className="font-medium text-charcoal-600">{form.email}</span>
            </p>
  ...
  const Steps = () => (
      <div className="flex items-center gap-3 mb-8">
        {["Your details", "Project details"].map((label, i) => {
          const n = i + 1; const active = step === n; const done = step > n;
          return (
              <div key={label} className="flex items-center gap-3">
                {i > 0 && <div className={`h-px flex-1 w-10 transition-colors duration-300 ${done ? "bg-orange-400" : "bg-charcoal-100"}`} />}
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-sm font-medium hidden sm:block transition-colors ${active ? "text-charcoal-900" : done ? "text-charcoal-500" : "text-charcoal-300"}`}>
                {label}
              </span>
                </div>
              </div>
  ...
  return (
      <>
        {/* Phone input — restyled to match .input-field exactly */}
        <style>{`
        /* Wrapper matches .input-field dimensions and border */
        ...
        .rpi-wrapper.error .PhoneInput {
          border-color: #fca5a5;
        }
        .rpi-wrapper.error .PhoneInput:focus-within {
          border-color: transparent;
      ...
      `}</style>
        ...
        <div className="pt-16">
            ...
            <span className="inline-flex items-center gap-1.5 text-orange-400 text-xs font-bold tracking-widest uppercase bg-orange-500/10 border border-orange-500/20 px-3 py-1.5 rounded-full mb-5">
              ...
              <div className="flex items-center justify-center gap-6 mt-8 flex-wrap">
                {[{ icon: "⚡", text: "2-hr response" }, { icon: "🔒", text: "Fully confidential" }, { icon: "📍", text: "Kenya-wide coverage" }].map(({ icon, text }) => (
                    <div key={text} className="flex items-center gap-2 text-charcoal-400 text-sm">
          ...
          {/* ── Form ── */}
          <section className="py-14 bg-charcoal-50">
                      ...
                      <div className="space-y-5">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                          <div>
                            ...
                            {/* rpi-wrapper applies custom styles; .error adds red ring */}
                        ...
                        <div className="pt-2">
                          <button type="button" onClick={nextStep} className="btn-primary w-full justify-center py-3.5 text-base">
                      ...
                      <div className="space-y-6">
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <button type="button" onClick={prevStep}
                                  className="flex items-center gap-1.5 text-sm font-medium text-charcoal-400 hover:text-charcoal-700 transition-colors flex-shrink-0 mt-1">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-1">
                            {SERVICE_TYPES.map(({ value, icon, label }) => (
                                <button key={value} type="button" onClick={() => set("serviceType", value)}
                                  ...
                                  <span className={`text-xs font-semibold leading-tight ${form.serviceType === value ? "text-orange-700" : "text-charcoal-700"}`}>
                              {label}
                            </span>
                                </button>
                            ))}
                        ...
                        <div>
                          ...
                          <textarea rows={5} value={form.description} onChange={e => set("description", e.target.value)}
                                    ...
                                    placeholder="Describe what needs verifying — the current stage, what has been done so far, materials or contractors involved, and what outcome you need. The more detail, the faster we match the right inspector."/>
                        ...
                        <div>
                          ...
                          <textarea rows={2} value={form.specificConcerns} onChange={e => set("specificConcerns", e.target.value)}
                                    ...
                                    placeholder="e.g. Contractor has missed milestones, suspect materials are substandard, title deed has disputed signatures…"/>
                          <p className="text-xs text-charcoal-400 mt-1.5">
                            Flag anything you want the inspector to pay particular attention to.
                          </p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                          <div>
                            ...
                            {/* react-select — identical styles to country picker */}
                            <Select
                            ...
                            <div className="flex flex-col gap-2">
                              {URGENCY.map(({ value, label, sub, dot }) => (
                                  <button key={value} type="button" onClick={() => set("urgency", value)}
                                    ...
                                    <span className={`text-sm font-semibold ${form.urgency === value ? "text-orange-700" : "text-charcoal-800"}`}>
                                {label}
                              </span>
                        ...
                        <div>
                          ...
                          <p className="text-xs text-charcoal-400 mt-1.5">
                            Anyone our inspector should coordinate with on arrival — caretaker, contractor, relative, or neighbour.
                          </p>
                        </div>

                        {/* What happens next */}
                        <div className="rounded-2xl bg-charcoal-950 p-5 flex gap-4">
                          <span className="text-2xl flex-shrink-0 mt-0.5">⚡</span>
                          <div>
                            <p className="font-semibold text-white text-sm mb-1">What happens next</p>
                            <p className="text-charcoal-400 text-sm leading-relaxed">
                              Our team reviews your request and contacts you within{" "}
                              <span className="text-orange-400 font-medium">2 business hours</span>{" "}
                              via email and WhatsApp to confirm the inspector, schedule the visit, and provide your quote.
                            </p>
                          </div>
                        </div>
                        ...
                        <p className="text-xs text-charcoal-400 text-center leading-relaxed">
                          ...
                          <a href="/privacy" className="underline hover:text-orange-600 transition-colors">Privacy Policy</a>.
