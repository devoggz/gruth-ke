...
export const metadata: Metadata = {
    ...
    description: 'Every verification service GRUTH offers — construction, land, events, business investment, and materials pricing.',
}
        ...
        <span className="inline-flex items-center gap-1.5 text-orange-400 text-xs font-bold tracking-widest uppercase bg-orange-400/10 border border-orange-400/20 px-3 py-1 rounded-full mb-6">
          What we do
                ...
                <p className="text-charcoal-400 text-lg leading-relaxed mb-10 max-w-xl mx-auto">
                    Five categories. One promise — a verified, photo-documented report from a qualified inspector on the ground in Kenya.
                </p>
                {/* Quick-nav pills */}
...
function ServiceDetail({ svc, index }: { svc: typeof SERVICES[0]; index: number }) {
    ...
    return (
        <section
            ...
            style={{ borderTop: isLight ? '1px solid #fed7aa' : '1px solid rgba(255,255,255,0.05)' }}
        >
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <span className="inline-flex items-center gap-1.5 bg-charcoal-950/70 backdrop-blur-sm border border-white/10 text-orange-400 text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full">
                {svc.tagline}
                    ...
                    {/* ── Content ───────────────────────────────────────────────────── */}
                    <div>
                        <h2 className={`font-display text-3xl sm:text-4xl font-bold tracking-tight mb-6 ${isLight ? 'text-charcoal-950' : 'text-white'}`}>
                            {svc.title}
                        </h2>

                        {/* What's included */}
                        <div className="mb-7">
                            <p className={`text-[10px] font-bold uppercase tracking-widest mb-3 ${isLight ? 'text-charcoal-400' : 'text-charcoal-500'}`}>
                                What's included
                            </p>
                            <ul className="space-y-3">
                                {svc.what.map((item, i) => (
                                    <li key={i} className={`flex items-start gap-3 text-sm leading-relaxed ${isLight ? 'text-charcoal-700' : 'text-charcoal-300'}`}>
                    <span className="mt-0.5 flex-shrink-0 w-4 h-4 rounded-full bg-orange-500/15 flex items-center justify-center">
                      <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="#f97316" strokeWidth="3">
                        ...
                        {/* Deliverable */}
                        <div className={`rounded-xl p-4 mb-7 ${isLight ? 'bg-orange-100 border border-orange-200' : 'bg-white/[0.05] border border-white/10'}`}>
                            <p className={`text-[10px] font-bold uppercase tracking-widest mb-1.5 ${isLight ? 'text-orange-500' : 'text-charcoal-500'}`}>
                                Deliverable
                            </p>
                            <p className={`text-sm leading-relaxed ${isLight ? 'text-charcoal-800' : 'text-charcoal-200'}`}>
                                {svc.deliverable}
                            </p>
                        </div>

                        {/* Ideal for */}
                        <p className={`text-xs leading-relaxed mb-7 ${isLight ? 'text-charcoal-500' : 'text-charcoal-500'}`}>
                            <span className={`font-bold ${isLight ? 'text-charcoal-700' : 'text-charcoal-400'}`}>Ideal for: </span>
                            {svc.ideal}
                        </p>
...
function CtaBanner() {
    ...
    return (
        <section className="bg-charcoal-950 py-20" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
