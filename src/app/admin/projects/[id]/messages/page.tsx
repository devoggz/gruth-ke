...
function Bubble({ msg }: { msg: Message }) {
    ...
    return (
        <div className={`flex ${isAdmin ? 'justify-end' : 'justify-start'} group`}>
            <div className={`max-w-[75%] flex flex-col ${isAdmin ? 'items-end' : 'items-start'} gap-1`}>
                {!isAdmin && (
...
export default function AdminProjectMessagesPage() {
    ...
    async function send() {
        ...
        const optimistic: Message = {
            id: `opt-${Date.now()}`, content: input.trim(),
            isFromClient: false, createdAt: new Date().toISOString(),
            readAt: null, senderName: 'GRUTH Team',
        }
        ...
        <div className="pb-12 max-w-3xl">
            <div className="mb-6">
                <div className="text-xs text-charcoal-400 mb-1">
                    <a href="/admin" className="hover:text-orange-600">Admin</a>
                    {' / '}<a href="/admin/projects" className="hover:text-orange-600">Projects</a>
                    {' / '}<a href={`/admin/projects/${projectId}`} className="hover:text-orange-600">{project?.name}</a>
                    {' / '}Messages
                </div>
