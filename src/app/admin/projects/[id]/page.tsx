        ...
        <div className="space-y-6 pb-16 max-w-6xl">
            ...
            <div>
                <div className="flex items-center gap-1.5 text-xs text-charcoal-400 mb-3">
                    ...
                    <span>/</span>
                    <Link href="/admin/projects" className="hover:text-orange-500 transition-colors">Projects</Link>
                    <span>/</span>
                    <span className="text-charcoal-600 font-medium truncate max-w-[200px]">{project.name}</span>
                </div>
                ...
                {project.status}
                    ...
                    <div className="flex items-center gap-2.5 flex-wrap">
                        ...
                        >
                            ...
                            {unreadMessages > 0 ? `${unreadMessages} unread` : 'Message client'}
                        </Link>
            ...
            {unreadAlerts > 0 && (
              ...
              <span className="text-lg flex-shrink-0">
                {alert.severity === 'CRITICAL' ? '🔴' : alert.severity === 'WARNING' ? '⚠️' : 'ℹ️'}
                            ...
                            <span className="text-[10px] text-charcoal-400 flex-shrink-0">{formatRelativeDate(alert.createdAt)}</span>
                        </div>
                    ))}
                </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-white rounded-2xl border border-charcoal-100 p-5">
                    <p className="text-[10px] font-bold text-charcoal-400 uppercase tracking-[0.12em] mb-4">Client</p>
              ...
              <span className="text-white text-sm font-bold">
                {project.client.name?.charAt(0).toUpperCase() ?? '?'}
                        ...
                        <div>
                            <div className="font-semibold text-charcoal-900">{project.client.name}</div>
                            <div className="text-xs text-charcoal-400">{project.client.email}</div>
                        </div>
                    </div>
                    <div className="space-y-2">
                        {project.client.phone && (
                            <a href={`tel:${project.client.phone}`}
                               className="flex items-center gap-2 text-xs text-charcoal-600 hover:text-orange-500 transition-colors">
                                <svg className="w-3.5 h-3.5 text-charcoal-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
                ...
                {/* Budget */}
                <div className="bg-white rounded-2xl border border-charcoal-100 p-5">
                    <p className="text-[10px] font-bold text-charcoal-400 uppercase tracking-[0.12em] mb-4">Budget</p>
                    {project.estimatedBudget ? (
                        <div className="space-y-3">
                            {[
                                { label: 'Estimated', value: formatCurrency(project.estimatedBudget, project.currency), color: 'text-charcoal-900' },
                                { label: 'Spent',     value: formatCurrency(project.amountSpent ?? 0, project.currency), color: 'text-charcoal-700' },
                                { label: 'Remaining', value: formatCurrency(project.estimatedBudget - (project.amountSpent ?? 0), project.currency), color: 'text-emerald-600' },
                                ...
                                <div className="pt-1">
                                    <div className="h-1.5 bg-charcoal-100 rounded-full overflow-hidden">
                                        <div
                                            className={`h-full rounded-full transition-all ${
                                                budget.level === 'low' ? 'bg-emerald-500'
                                                    : budget.level === 'medium' ? 'bg-amber-500'
                                                        : 'bg-red-500'
                                            }`}
                                    ...
                                    <div className="flex justify-between mt-1.5">
                                        ...
                                        <span className={`text-xs font-semibold ${
                                            budget.level === 'low' ? 'text-emerald-600'
                                                : budget.level === 'medium' ? 'text-amber-600'
                                                    : 'text-red-600'
                                        }`}>{budget.label}</span>
                                    </div>
                                </div>
                ...
                <div className="bg-white rounded-2xl border border-charcoal-100 p-5">
                    <div className="flex items-center justify-between mb-4">
                        <p className="text-[10px] font-bold text-charcoal-400 uppercase tracking-[0.12em]">Progress</p>
                        {project.progressStages.length > 0 && (
                            <span className="text-xs font-bold text-charcoal-700">{progressPct}%</span>
                        )}
                    </div>
                    {project.progressStages.length > 0 ? (
                        <>
                            ...
                            <div className="space-y-2.5">
                                {project.progressStages.map(stage => (
                                    <div key={stage.id} className="flex items-center gap-3">
                                        ...
                                        <span className={`text-xs flex-1 ${stage.completed ? 'text-charcoal-700 font-medium' : 'text-charcoal-400'}`}>
                      {stage.stageName}
                    </span>
                                        {stage.completedAt && (
                                            <span className="text-[10px] text-charcoal-300">{formatDate(stage.completedAt)}</span>
                                        )}
                                    </div>
                                ))}
                            </div>
            ...
            {project.description && (
                <div className="bg-white rounded-2xl border border-charcoal-100 p-5">
                    <p className="text-[10px] font-bold text-charcoal-400 uppercase tracking-[0.12em] mb-3">Project Description</p>
            ...
            <div className="bg-white rounded-2xl border border-charcoal-100 overflow-hidden">
                <div className="px-6 py-4 border-b border-charcoal-100 flex items-center justify-between">
                    <div>
                        <h2 className="font-display font-semibold text-charcoal-950">Inspection Timeline</h2>
                        <p className="text-xs text-charcoal-400 mt-0.5">{project.inspections.length} inspection{project.inspections.length !== 1 ? 's' : ''}</p>
                    </div>
                </div>
                <div className="px-6 py-5">
                    {project.inspections.length === 0 ? (
                        <p className="text-sm text-charcoal-300 py-8 text-center">No inspections recorded yet.</p>
                    ) : (
                            ...
                            <div className="space-y-8">
                                {project.inspections.map(inspection => (
                                    <div key={inspection.id} className="relative pl-10">
                                        <div className={`absolute left-0 top-1.5 w-3.5 h-3.5 rounded-full border-2 border-white shadow-sm ${
                                        ...
                                        <div className="flex items-start justify-between gap-4 flex-wrap">
                                            ...
                                            <span className={`status-badge text-[10px] ${getStatusColor(inspection.status)}`}>
                        {inspection.status}
                      </span>
                                        </div>
                                        {inspection.summary && (
                                        ...
                                        {inspection.observations && (
                                            <p className="text-charcoal-500 text-xs mt-2 leading-relaxed pl-1">
                                                <span className="font-semibold text-charcoal-600">Observations: </span>
                                                {inspection.observations}
                                            </p>
                                        )}
                                        {inspection.media.length > 0 && (
                                            <div className="flex gap-2 mt-3 flex-wrap">
                                                {inspection.media.filter(m => m.type === 'PHOTO').slice(0, 6).map(media => (
                                                    <div key={media.id} className="relative w-16 h-16 rounded-lg overflow-hidden bg-charcoal-100 flex-shrink-0">
                                                        <Image
                                                            src={media.url}
                                        ...
                                        {inspection.report && (
                                            <a href={inspection.report.fileUrl ?? '#'}
            ...
            {allPhotos.length > 0 && (
                <div className="bg-white rounded-2xl border border-charcoal-100 p-5">
                    <p className="text-[10px] font-bold text-charcoal-400 uppercase tracking-[0.12em] mb-4">
                        Evidence Gallery · {allPhotos.length} photo{allPhotos.length !== 1 ? 's' : ''}
                    </p>
                    <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-2">
                        {allPhotos.map(photo => (
                            <div key={photo.id} className="group relative aspect-square rounded-xl overflow-hidden bg-charcoal-100">
                                <Image
                                    src={photo.url}
                                ...
                                {photo.caption && (
                                    <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent p-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <p className="text-white text-[9px] leading-tight">{photo.caption}</p>
                                    </div>
                                )}
                            </div>
                        ))}
            ...
            {project.materialPrices.length > 0 && (
                <div className="bg-white rounded-2xl border border-charcoal-100 overflow-hidden">
                    <div className="px-6 py-4 border-b border-charcoal-100">
                        <h2 className="font-display font-semibold text-charcoal-950">Material Price Intelligence</h2>
                    </div>
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tr className="border-b border-charcoal-100 bg-charcoal-50/50">
                                {['Material', 'Unit', 'Market Range', 'Quoted', 'Verified', 'Supplier', 'Status'].map(h => (
                                    <th key={h} className="px-5 py-3 text-left text-[10px] font-bold text-charcoal-400 uppercase tracking-[0.1em] whitespace-nowrap">
                                        {h}
                                    </th>
                                ))}
                            </tr>
                            ...
                            {project.materialPrices.map(item => (
                                <tr key={item.id} className="hover:bg-charcoal-50/30 transition-colors">
                                    <td className="px-5 py-3.5 font-medium text-charcoal-900">{item.materialName}</td>
                                    <td className="px-5 py-3.5 text-charcoal-500">{item.unit}</td>
                                    ...
                                    <td className="px-5 py-3.5 text-charcoal-500 text-xs">{item.supplier?.name ?? '—'}</td>
                                    <td className="px-5 py-3.5">
                                        <span className={`status-badge text-[10px] ${getStatusColor(item.status)}`}>{item.status}</span>
                                    </td>
                                </tr>
                            ))}
                            </tbody>
                ...
                <div className="bg-white rounded-2xl border border-charcoal-100 p-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {project.vendors.map(vendor => (
                            <div key={vendor.id} className="border border-charcoal-100 rounded-xl p-4 hover:border-charcoal-200 transition-colors">
                                <div className="flex items-start justify-between gap-2 mb-2">
                                    <div>
                                        <div className="font-semibold text-charcoal-900 text-sm">{vendor.name}</div>
                                        <div className="text-orange-500 text-xs font-medium mt-0.5">{vendor.role}</div>
                                    </div>
                                    <span className={`status-badge text-[10px] ${getStatusColor(vendor.verifiedStatus)}`}>
                    {vendor.verifiedStatus}
                  </span>
                                </div>
                                <div className="space-y-1 mt-3">
                                    {vendor.phone && (
                                        <a href={`tel:${vendor.phone}`} className="block text-xs text-charcoal-500 hover:text-orange-500 transition-colors">
                                            📞 {vendor.phone}
                                        </a>
                                    )}
                                    {vendor.email && (
                                        <a href={`mailto:${vendor.email}`} className="block text-xs text-charcoal-500 hover:text-orange-500 transition-colors">
                                            ✉️ {vendor.email}
                                        </a>
                                    )}
            ...
            {project.alerts.length > 0 && (
                <div className="bg-white rounded-2xl border border-charcoal-100 overflow-hidden">
                    <div className="px-6 py-4 border-b border-charcoal-100">
                        <h2 className="font-display font-semibold text-charcoal-950">Alert Log</h2>
                        <p className="text-xs text-charcoal-400 mt-0.5">{project.alerts.length} total · {unreadAlerts} unread</p>
                    </div>
                    <div className="divide-y divide-charcoal-50">
                        {project.alerts.map(alert => (
                            <div key={alert.id} className={`px-6 py-3.5 flex items-start gap-3 transition-colors ${alert.isRead ? 'hover:bg-charcoal-50/30' : 'bg-orange-50/20 hover:bg-orange-50/30'}`}>
                    ...
                    <span className={`text-sm font-semibold ${alert.isRead ? 'text-charcoal-600' : 'text-charcoal-900'}`}>
                      {alert.title}
                    </span>
                                        <span className="text-[10px] text-charcoal-400 flex-shrink-0">{formatRelativeDate(alert.createdAt)}</span>
                                    </div>
                                    <p className="text-xs text-charcoal-500 mt-0.5 leading-relaxed">{alert.message}</p>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <div className="bg-white rounded-2xl border border-charcoal-100 overflow-hidden">
                <div className="px-6 py-4 border-b border-charcoal-100 flex items-center justify-between">
                    <div>
                        <h2 className="font-display font-semibold text-charcoal-950">Messages</h2>
                        <p className="text-xs text-charcoal-400 mt-0.5">
                            {unreadMessages > 0 ? `${unreadMessages} unread from client` : 'Recent thread'}
                        </p>
                    </div>
                    <Link href={`/admin/projects/${project.id}/messages`}
                          className={`inline-flex items-center gap-1.5 text-xs font-semibold transition-colors ${
                              unreadMessages > 0 ? 'text-violet-600 hover:text-violet-700' : 'text-orange-500 hover:text-orange-600'
                          }`}>
                        {unreadMessages > 0 ? `Reply (${unreadMessages} unread)` : 'Open thread'}
                        ...
                        {project.messages.map(msg => (
                            <div key={msg.id} className={`flex ${msg.isFromClient ? 'justify-start' : 'justify-end'}`}>
                                <div className={`max-w-[80%] px-4 py-2.5 rounded-2xl text-sm ${
                                    ...
                                    {!msg.isFromClient && (
                                        <div className="text-[10px] font-bold text-violet-200 mb-1 uppercase tracking-wide">GRUTH Team</div>
                                    )}
                                    <p className="leading-relaxed">{msg.content}</p>
                                    <div className={`text-[10px] mt-1.5 ${msg.isFromClient ? 'text-charcoal-400' : 'text-violet-300'}`}>
