    ...
    return (
        <div className="space-y-6 pb-16">
            ...
            <div className="flex items-start justify-between gap-4 flex-wrap">
                <div>
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-[10px] font-bold text-charcoal-400 uppercase tracking-[0.12em]">Live</span>
                    </div>
                    ...
                    <p className="text-charcoal-400 text-sm mt-2">
                        {clients.length} clients · {projects.length} projects · {totalRequestsCount} intake submissions
                    </p>
                </div>
                <div className="flex gap-2">
                    <Link href="/dashboard/market-prices"
                        ...
                        <svg className="w-3.5 h-3.5 text-charcoal-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                {([
                ...
                ] as const).map(({ label, value, sub, href, dot, urgent }) => (
                    <Link key={label} href={href}
                          className={`group bg-white rounded-2xl border p-5 transition-all duration-200 hover:shadow-md hover:-translate-y-px cursor-pointer ${
                              urgent ? 'border-orange-200 ring-1 ring-orange-100' : 'border-charcoal-100 hover:border-charcoal-200'
                          }`}>
                        <div className="flex items-center gap-2 mb-4">
                            <div className={`w-2 h-2 rounded-full ${dot} ${urgent && value > 0 ? 'animate-pulse' : ''}`} />
                            <span className="text-[10px] font-bold text-charcoal-400 uppercase tracking-[0.1em]">{label}</span>
                        </div>
                        ...
                        <div className="flex items-center justify-between mt-3">
                            ...
                            <svg className="w-3.5 h-3.5 text-charcoal-200 group-hover:text-orange-400 transition-colors" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-white rounded-2xl border border-charcoal-100 p-5">
                    <p className="text-[10px] font-bold text-charcoal-400 uppercase tracking-[0.12em] mb-5">Project Status</p>
                    <div className="space-y-3.5">
                        {[
                            { label: 'Active',    count: activeProjects,    bar: 'bg-emerald-500', txt: 'text-emerald-600' },
                            { label: 'Pending',   count: pendingProjects,   bar: 'bg-amber-400',   txt: 'text-amber-600'  },
                            { label: 'On Hold',   count: onHoldProjects,    bar: 'bg-orange-400',  txt: 'text-orange-600' },
                            { label: 'Completed', count: completedProjects, bar: 'bg-blue-400',    txt: 'text-blue-600'   },
                        ].map(({ label, count, bar, txt }) => {
                            const pct = projects.length ? Math.round((count / projects.length) * 100) : 0
                            return (
                                <div key={label} className="flex items-center gap-3">
                                    <span className="text-[11px] text-charcoal-500 w-16 flex-shrink-0">{label}</span>
                                    <div className="flex-1 bg-charcoal-100 rounded-full h-1.5 overflow-hidden">
                                        <div className={`h-full rounded-full transition-all ${bar}`} style={{ width: `${pct}%` }} />
                                    </div>
                                    <span className={`text-xs font-bold tabular-nums w-4 text-right ${txt}`}>{count}</span>
                                </div>
                            )
                        })}
                    </div>
                ...
                {/* Service mix */}
                <div className="bg-white rounded-2xl border border-charcoal-100 p-5">
                    <p className="text-[10px] font-bold text-charcoal-400 uppercase tracking-[0.12em] mb-5">Service Mix</p>
                    <div className="space-y-3">
                        {serviceBreakdown.length > 0 ? serviceBreakdown.slice(0, 5).map(([type, count]) => (
                            <div key={type} className="flex items-center gap-3">
                                <span className="text-[11px] text-charcoal-600 truncate flex-1">{getProjectTypeLabel(type)}</span>
                                ...
                                <span className="text-xs font-bold text-charcoal-500 tabular-nums w-4 text-right">{count}</span>
                            </div>
                        )) : (
                            <p className="text-xs text-charcoal-300 py-6 text-center">No projects yet</p>
                        )}
                    </div>
                </div>
                ...
                <div className="bg-charcoal-950 rounded-2xl p-5">
                    <p className="text-[10px] font-bold text-charcoal-600 uppercase tracking-[0.12em] mb-4">Quick Actions</p>
                    <div className="space-y-1">
                        {[
                            { label: 'All Projects',    href: '/admin/projects',          emoji: '📁' },
                            { label: 'All Clients',     href: '/admin/clients',           emoji: '👥' },
                            { label: 'Intake Requests', href: '/admin/requests',          emoji: '📋' },
                            { label: 'Messages',        href: '/admin/messages',          emoji: '💬' },
                            { label: 'Market Prices',   href: '/dashboard/market-prices', emoji: '📊' },
                            ...
                            <Link key={label} href={href}
                                ...
                                <svg className="w-3 h-3 text-charcoal-700 group-hover:text-charcoal-400 transition-colors" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                                    <path d="M9 18l6-6-6-6"/>
                                </svg>
                            </Link>
                        ))}
            ...
            <div className="bg-white rounded-2xl border border-charcoal-100 overflow-hidden">
                <div className="px-6 py-4 border-b border-charcoal-100 flex items-center justify-between">
                    <div>
                        <h2 className="font-display font-semibold text-charcoal-950">Recent Projects</h2>
                        <p className="text-xs text-charcoal-400 mt-0.5">{projects.length} total · showing latest 10</p>
                    </div>
                ...
                {projects.length === 0 ? (
                    <div className="py-20 text-center">
                        <div className="w-12 h-12 bg-charcoal-50 rounded-2xl flex items-center justify-center mx-auto mb-3">
                            <svg className="w-5 h-5 text-charcoal-200" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            ...
                            <tr className="border-b border-charcoal-100 bg-charcoal-50/50">
                                {['Project', 'Client', 'Type', 'Status', 'Last Inspection', 'Budget', 'Alerts', ''].map(h => (
                            ...
                            {projects.slice(0, 10).map(project => (
                                <tr key={project.id} className="hover:bg-orange-50/15 group transition-colors">
                                    <td className="px-5 py-3.5">
                                        <div className="font-semibold text-charcoal-900 text-sm truncate max-w-[160px]">{project.name}</div>
                                    </td>
                                    <td className="px-5 py-3.5">
                          ...
                          <span className="text-white text-[9px] font-bold">
                            {project.client.name?.charAt(0).toUpperCase() ?? '?'}
                                            ...
                                            <span className="text-charcoal-600 text-xs truncate max-w-[100px]">{project.client.name}</span>
                                        </div>
                                    </td>
                                    <td className="px-5 py-3.5">
                                        <span className="text-charcoal-400 text-xs">{getProjectTypeLabel(project.type)}</span>
                                    </td>
                                    <td className="px-5 py-3.5">
                      <span className={`status-badge text-[10px] ${getStatusColor(project.status)}`}>
                        {project.status}
                      </span>
                                    </td>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                ...
                <div className="bg-white rounded-2xl border border-charcoal-100 overflow-hidden">
                    <div className="px-5 py-4 border-b border-charcoal-100 flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-xs text-charcoal-400 mt-0.5">{clients.length} registered accounts</p>
                        </div>
                    ...
                    {clients.length === 0 ? (
                        <div className="py-16 text-center text-sm text-charcoal-300">No clients yet.</div>
                    ) : (
                        <div className="divide-y divide-charcoal-50">
                            {clients.slice(0, 6).map(client => {
                                const active = client.projects.filter(p => p.status === 'ACTIVE').length
                                return (
                                    <Link key={client.id} href="/admin/clients"
                      ...
                      <span className="text-white text-xs font-bold">
                        {client.name?.charAt(0).toUpperCase() ?? '?'}
                                        ...
                                        <div className="text-right flex-shrink-0">
                                            ...
                                            {active > 0 && (
                                                <div className="text-[10px] text-emerald-600 font-semibold">{active} active</div>
                                            )}
                                        </div>
                                    </Link>
                                )
                ...
                <div className="bg-white rounded-2xl border border-charcoal-100 overflow-hidden">
                    <div className="px-5 py-4 border-b border-charcoal-100">
                        <h2 className="font-display font-semibold text-charcoal-950">Recent Alerts</h2>
                        <p className="text-xs text-charcoal-400 mt-0.5">Last {recentAlerts.length} sent</p>
                    </div>
                    {recentAlerts.length === 0 ? (
                        <div className="py-16 text-center text-sm text-charcoal-300">No alerts sent yet.</div>
                    ) : (
                        <div className="divide-y divide-charcoal-50">
                            {recentAlerts.map(alert => (
                                <div key={alert.id} className="px-5 py-3.5 flex items-start gap-3 hover:bg-charcoal-50/30 transition-colors">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <span className={`inline-flex items-center text-[9px] font-bold px-1.5 py-0.5 rounded-full mt-1.5 uppercase tracking-wide ${
